...
try {
  if (!process.env.OPENAI_API_KEY) {
    console.error("[OPENAI_ERROR] OPENAI_API_KEY is not set in environment variables");
  }
  openai = new OpenAI({
...
`;
  ...
  try {
    ...
    if (error.message?.includes("API key")) {
      ...
      throw new Error("OpenAI API quota exceeded. Please check your billing and quota limits.");
