...
import { generateJobDescription } from "../utils/gemini.service.js";
...
const getJobs = asyncHandler(async (req, res) => {
  ...
  // This part is used to show jobs page by page (pagination)
  const page = parseInt(req.query.page, 10) || 1;
  // If not provided, default to 10 jobs per page.
  const limit = parseInt(req.query.limit, 10) || 10;
  // Calculate the starting index
  const startIndex = (page - 1) * limit;
  const endIndex = page * limit;
  // Count total number of job documents in the database that match the query.
  ...
  // Pagination result
  // If there are more jobs after the current page, show "next" page info
  const pagination = {};
  if (endIndex < total) {
    pagination.next = {
      page: page + 1,
...
const sendJobDescription = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!response) {
      throw new ApiError(500, "Failed to generate job description. Please try again.");
    }

    // Decrement quota only if description was successfully generated
    ...
    throw new ApiError(500, error.message || "Failed to generate job description. Please try again.");
  }
});
