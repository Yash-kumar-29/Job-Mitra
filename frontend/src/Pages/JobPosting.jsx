...
function JobPosting() {
  ...
  const handleGenerate = async () => {
    ...
    // Check required fields for AI generation
    const requiredFields = [
      { key: 'title', label: 'Title' },
      { key: 'type', label: 'Type of position' },
      ...
      { key: 'benefits', label: 'Benefits', isArray: true },
      { key: 'numberOfOpenings', label: 'Number of openings', isNumber: true },
      { key: 'salaryRange', label: 'Salary range', isObject: true },
    ];
    ...
    for (let field of requiredFields) {
      ...
      if (field.isArray) {
        ...
        isEmpty = !value || (value.from === 0 && value.to === 0);
      ...
      if (isEmpty) {
        setDialog({
          ...
          message: `Please fill in the ${field.label} field to generate job description.`,
          buttonText: "OK",
        });
        return;
      }
    ...
    try {
      const generatedDescription = await companyService.generateJobDescription(formData);
      ...
      if (error?.response?.data?.message?.includes("Quota exceeded")) {
        ...
        setDialog({
          ...
          message: error?.response?.data?.message || "An error occurred while generating the job description. Please try again.",
          buttonText: "OK",
        });
      }
    }
